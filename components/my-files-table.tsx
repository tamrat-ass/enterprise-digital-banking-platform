...
import { fetchMyFiles } from '@/app/actions/documents'
...
export function MyFilesTable() {
  ...
  const fetchFiles = async () => {
    try {
      ...
      logger.debug('[MyFilesTable] Calling server action to fetch my files only')
      // Use fetchMyFiles instead of fetchDocuments to get only current user's files
      ...
      if (!result.success) {
        console.error('[MyFilesTable] Server action error:', result.error)
      ...
      logger.debug('[MyFilesTable] My files from server action:', filesList)
      ...
      console.error('[MyFilesTable] Error:', errorMsg)
      setError(errorMsg)
      setFiles([])
    } finally {
      setLoading(false)
  ...
  // Empty state when user has no files
  if (filteredFiles.length === 0 && !searchTerm && !filterStatus) {
    return (
      <div className="space-y-6">
        <div className="bg-white border border-[#E6E6E6] rounded-2xl shadow-sm p-12 text-center">
          ...
          <a 
            href="/upload"
            className="inline-block px-6 py-2 bg-[#6B4423] text-white rounded-lg hover:bg-[#5A3819] transition-colors"
          >
